...
import pe.edu.upeu.asistenciaupeujc.ui.presentation.screens.actividad.AsistenciapaForm
import pe.edu.upeu.asistenciaupeujc.ui.presentation.screens.actividad.AsistenciapaUI
...
fun NavigationHost(
    ...
    NavHost(
        ...
        composable(Destinations.AsistenciapaUI.route){
            AsistenciapaUI(navegarEditarAsis = {newText->navController.navigate(Destinations.AsistenciapaForm.passId(newText))}, navController =navController )
        }

        composable(Destinations.AsistenciapaForm.route, arguments = listOf(navArgument("asisId"){
            defaultValue="asisId"
        })){
                navBackStackEntry -> var asisId=navBackStackEntry.arguments?.getString("asisId")
            requireNotNull(asisId)
            AsistenciapaForm(text = asisId, darkMode = darkMode, navController =navController )
        }
